...
def train_and_evaluate(embeddings_train, labels_train, embeddings_val, labels_val, embeddings_test, labels_test):
    ...
    for clf_name, clf_class in CLASSIFIERS.items():
        ...
        metrics = {
            ...
            "FineTuningTime": time.time() - start_fine_tune
        }
...
def run_experiments(models=MODELS):
    ...
    for model_name in tqdm(models, desc="Processing models"):
        for percent in range(5, 100, 5):
            ...
            print(f"\nProcessing model: {model_name}, {percent}% training data")
            ...
            train_embeddings = generate_embeddings(model_name, sampled_train_texts)
            ...
            model_results = train_and_evaluate(
                train_embeddings, sampled_train_labels,
            ...
            for result in model_results:
                result.update({
                    ...
                    "TotalTime": embedding_time + result["FineTuningTime"],
                    "TrainingDataPercent": percent
                })
                final_results.append(result)
                
            pd.DataFrame(final_results).to_csv(
                f"Results/embedding_classification_ablation_intermediate.csv",
                index=False
            )
...
if __name__ == "__main__":
    ...
    results_df.to_csv("Results/embedding_classification_ablation_results.csv")
